...
export function useListRuntime({ 
  ...
  const [columnConfigs, setColumnConfigs] = useState<Record<string, unknown>>(() => {
    const saved = localStorage.getItem(`buopso.list.${doctype}.columns`);
    return saved ? JSON.parse(saved) : {};
  });

  const saveColumnConfig = useCallback((colId: string, config: Record<string, unknown>) => {
    setColumnConfigs(prev => {
      ...
      localStorage.setItem(`buopso.list.${doctype}.columns`, JSON.stringify(updated));
      return updated;
    });
  }, [doctype]);
   ...
   */
  const handleCellChange = useCallback(async ({ rowIndex, colId, newValue, row }: {
    rowIndex: number;
    colId: string;
    newValue: unknown;
    row: ListRow;
    ...
    try {
      ...
      console.error("[buopso] Cell update failed:", err);
   ...
   * Action Orchestrator
   */
  const executeAction = useCallback((action: string, rows: ListRow[]) => {
