...
export type CommandHandler<T = unknown> = (command: Command<T>) => Promise<unknown>;
...
class CommandBus {
  ...
  public async dispatch<T>(command: Command<T>): Promise<unknown> {
    console.log(`[buopso] Dispatching command: ${command.type}`, command.payload);
    ...
    if (container.has("ability")) {
      ...
      if (ability.hasRules() && !ability.can("execute", command.type)) {
        throw new Error(`[buopso] Permission denied for command: ${command.type}`);
      }
    }
    ...
    if (!handler) {
      throw new Error(`[buopso] No handler registered for command: ${command.type}`);
    }
    ...
    try {
      ...
      console.log(`[buopso] Command ${command.type} completed successfully.`);
      
      return result;
    } catch (err) {
      console.error(`[buopso] Command ${command.type} failed:`, err);
      throw err;
    }
  }
}
